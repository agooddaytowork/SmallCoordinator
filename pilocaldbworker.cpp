...
piLocalDBWorker::piLocalDBWorker(QObject *parent) :
    QObject(parent), tmpBP(BinaryProtocol(0)), tmpWP(WindowProtocol(0)),
...
void piLocalDBWorker::updatePVIData(quint32 GlobalID, double Pressure, double Voltage, double Current)
{
    anIf(piLclDbWrkDbgEn, anTrk("GATE"));
    tmpStr1 = QString::number(GlobalID);
    tmpStr2 = QString::number(Pressure);
    tmpStr3 = QString::number(Voltage);
    tmpStr4 = QString::number(Current);
