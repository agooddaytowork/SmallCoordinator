...
UHV4PVICollectorDB::UHV4PVICollectorDB(QObject *parent) : QObject(parent), currentWP(new WindowProtocol(0))
{
...
bool UHV4PVICollectorDB::gotoNextRecord()
{
    ...
    currentWPNo = currentQuery.value("pumpAddr").toInt();
    currentCH = currentQuery.value("pumpCH").toInt();
    currentWP->SetWPNo(currentWPNo);
//    currentWP(currentCH);
...
void UHV4PVICollectorDB::emitReadP()
{
    ...
    {
        currentWP->ReadP();
        emit CommandOut(currentWP->GenerateMSG());
    }
    else
    {
...
void UHV4PVICollectorDB::saveP()
{
    ...
    {
        currentWP = &WindowProtocol::WC(currentReplyFromPump);
        currentPressure = QString(currentWP->GetDATA());
    }
...
void UHV4PVICollectorDB::saveV()
{
    ...
    {
        currentWP = &WindowProtocol::WC(currentReplyFromPump);
        currentVoltage = QString(currentWP->GetDATA());
    }
...
void UHV4PVICollectorDB::saveI()
{
    ...
    {
        currentWP = &WindowProtocol::WC(currentReplyFromPump);
        currentCurrent = QString(currentWP->GetDATA());
    }
